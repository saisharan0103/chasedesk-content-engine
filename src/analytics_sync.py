...
(Bookmarks / url_clicks map to Typefully's `saves` / `link_clicks` and may be
null on X's side; we leave the cell blank when so.)

This is what makes `python -m src.review retune` work end-to-end with no manual
data entry.
"""
...
_METRIC_MAP = {
    "likes": "likes",
    "comments": "replies",      # X replies count
    "shares": "reposts",        # X reposts
    "saves": "bookmarks",       # may be null on Typefully's side
...
def sync_metrics(config: Config | None = None, *, client: TypefullyClient | None = None) -> dict:
    """Walk metrics.csv, fetch analytics, write metrics back. Returns a small summary dict."""
    ...
    for row in rows:
        ...
        for src_field, dest_col in _METRIC_MAP.items():
            if dest_col not in row:
                continue
            if not _is_blank(row.get(dest_col)):
                continue
