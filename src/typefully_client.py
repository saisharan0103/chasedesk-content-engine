"""Typefully API v2 client.

Auth: ``Authorization: Bearer <api_key>``  (not ``X-API-KEY`` — that's v1 and is
rejected for new keys.)
...
class TypefullyClient:
    ...
    def _request(self, method: str, path: str, *, json_body: dict | None = None, params: dict | None = None) -> dict:
        ...
        resp = self._http().request(
            method, url, json=json_body, params=params, headers=self._headers(), timeout=self.timeout
        )
    ...
    def ensure_social_set_id(self) -> str:
        ...
        if not sets:
            raise TypefullyError("no social sets found on this Typefully account")
        first = sets[0]
    ...
    def create_draft(
        ...
        *,
        ...
        share: bool = True,
        threadify: bool = False,  # kept for orchestrator parity; v2 threads via the posts array
        ...
        if schedule_date:
            body["publish_at"] = schedule_date  # ISO 8601 UTC, "now", or "next-free-slot"
        if draft_title:
