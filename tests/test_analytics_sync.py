...
def _make_metrics_csv(tmp_path: Path, draft_id: str) -> Path:
    ...
    entries = [
        RunEntry(
            slot=0, posted_for="2026-05-17", scheduled_local="2026-05-17 10:00 IST",
            scheduled_utc="2026-05-17T04:30:00Z", status="scheduled",
            strategy="pain_led_story", strategy_name="Pain-led story",
            shape="two_part", segment="solo_bookkeeper",
            seed_id="monday_email_queue", competitor_id=None,
            hook_type="operational_pain", tweet_text="some text",
            character_count=9, quality_passed=True, needs_human_review=False,
            regen_count=0, generator="openai", typefully_draft_id=draft_id,
        )
    ]
    append_metrics(logs_dir, date(2026, 5, 16), entries)
